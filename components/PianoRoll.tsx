...
export default function PianoRoll({ trackIndex, trackName, notes, onAddNote, onRemoveNote, onUpdateDuration }: Props) {
    const isDrum = trackIndex < 3;
    const pitches = isDrum ? DRUM_PITCHES : BASS_PITCHES;
    const color = TRACK_COLORS[trackIndex] ?? "#6366f1";

    // Track which cell is being dragged to extend
    ...
    const handleCellMouseDown = (pitch: string, step: number) => {
        ...
        if (existing) {
            // Remove if clicking the start cell; drag-extend starts from right edge
    ...
    const handleDragStart = (e: React.MouseEvent, note: PianoRollNote) => {
        ...
        const onMove = (ev: MouseEvent) => {
            ...
            const cellWidth = 28; // approximate px per step cell
    ...
    return (
        <div className="flex flex-col border-t border-zinc-700 bg-zinc-950 select-none overflow-hidden">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 border-b border-zinc-800 bg-zinc-900">
                ...
                <span className="text-[10px] text-zinc-600 ml-auto">click = add/remove · drag right edge = extend</span>
            </div>

            {/* Grid */}
            <div className="flex overflow-y-auto overflow-x-auto" style={{ maxHeight: isDrum ? "56px" : "220px" }}>
                {/* Piano keys column */}
                <div className="flex flex-col shrink-0 border-r border-zinc-800" style={{ width: isDrum ? 56 : 48 }}>
                    {pitches.map(pitch => {
