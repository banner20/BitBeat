...
export default function Playground() {
    ...
    useEffect(() => {
        // Initialize Yjs bindings with the Liveblocks room context
        initSync(room);
    ...
    useEffect(() => {
        ...
        const startAudio = async () => {
            // Setup the sequencer but don't start it playing yet
            setupSequencer(gridRef, (step) => {
                setCurrentStep(step);
            });
        };
        startAudio();
    ...
    const togglePlay = async () => {
        await initAudio(); // Ensure audio is ready after user gesture
        if (!isPlaying) {
    ...
    return (
        <div
            className="flex flex-col h-screen w-full bg-zinc-950 text-white font-sans overflow-hidden relative"
            ...
            <InstrumentDock
                onRandomize={() => {
                    const newGrid = Array.from({ length: 4 }, (_, t) => {
                        const probs = [0.4, 0.2, 0.7, 0.3]; // kick, snare, hihat, bass
